...
export const Route = createFileRoute("/auth")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "سجّل الدخول أو أنشئ حساباً جديداً في منصة الإمام الأكبر التعليمية.",
      },
    ],
  }),
  component: AuthPage,
    ...
    try {
      if (mode === "login") {
        ...
        toast.success("تم إنشاء حسابك بنجاح!", {
          description: "برجاء تفعيل حسابك من رسالة البريد إن طُلب ذلك.",
        });
  ...
  return (
    <div className="relative min-h-screen flex flex-col items-center justify-center px-4 overflow-hidden bg-background">
      ...
      >
        ...
        <div className="glass-card rounded-2xl p-6 md:p-8 border border-border/50 shadow-glow">
          ...
          <div className="flex gap-2 p-1 rounded-xl bg-muted/30 mb-4">
            <button
              ...
              className={`flex-1 py-2 rounded-lg text-sm font-body transition-all flex items-center justify-center gap-2 ${mode === "login" ? "bg-gradient-to-r from-amber-600 to-amber-500 text-white shadow" : "text-muted-foreground"}`}
            >
            ...
            <button
              ...
              className={`flex-1 py-2 rounded-lg text-sm font-body transition-all flex items-center justify-center gap-2 ${mode === "register" ? "bg-gradient-to-r from-emerald-600 to-emerald-500 text-white shadow" : "text-muted-foreground"}`}
            >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button
              type="submit"
              disabled={loading}
              className={`w-full h-12 text-white ${mode === "login" ? "bg-gradient-to-r from-amber-600 to-amber-500 hover:from-amber-500 hover:to-amber-400" : "bg-gradient-to-r from-emerald-600 to-emerald-500 hover:from-emerald-500 hover:to-emerald-400"}`}
            >
              {loading ? (
                <>
        ...
        <Link
          to="/"
          className="mt-6 inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground font-body w-full justify-center"
        >
